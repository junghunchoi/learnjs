...
const uploadAndTrackFiles = (() => {
  const files = new Map(); // 파일별로 관리하기 위한 변수
  const progressBox = document.createElement('div'); // 진행상태 창
  const FILE_STATUS = {
    PENDING: 'pending',
    UPLOADING: 'uploading',
    PAUSED: 'paused',
  ...
  } // 파일별로 업로드상황에 따라 얼마나 처리되어있는지 표시

  const updateFileElement = fileObject => {// 파일별로 어떻게 처리되었는지 행별로 체크
    console.log(fileObject.element)
    const [
    ...
    requestAnimationFrame(() => {
      ...
      progressBar.style.width = fileObject.percentage + '%'; // !!여기서 진행상태 표시
...
//Please recommend a variable name to hold the file to upload.